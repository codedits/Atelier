          ...
          <div className="p-6">
            <div className="space-y-0">
              {[
                ...
                <div key={index} className={`flex justify-between items-center py-3.5 ${index > 0 ? 'border-t border-[#1a1a1a]' : ''}`}>
            ...
            {stats.lowStockProducts.length === 0 ? (
              ...
              <div className="space-y-0">
                {stats.lowStockProducts.slice(0, 5).map((product, index) => (
                  <div key={product.id} className={`flex justify-between items-center py-3.5 ${index > 0 ? 'border-t border-[#1a1a1a]' : ''}`}>
                    <span className="text-[#888] text-sm truncate mr-4">{product.name}</span>
                    <span className={`text-sm font-semibold px-2.5 py-1 rounded-lg ${product.stock === 0 ? 'bg-[#ff4444]/10 text-[#ff6166]' :
