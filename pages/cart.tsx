...
export default function CartPage() {
  ...
  if (items.length === 0) {
    return (
      <>
        <Head>
          <title>Shopping Cart â€” Atelier</title>
        </Head>
  ...
  return (
    <>
      <Head>
        <title>Shopping Cart ({totalItems}) â€” Atelier</title>
      </Head>
      <div className="min-h-screen bg-white">
        <Header />
        <main className="pt-24 pb-20">
          <div className="max-w-6xl mx-auto px-6 lg:px-8">
            <h1
              className="text-3xl font-medium text-[#111827] mb-8 animate-in fade-in slide-in-from-bottom-4 duration-500"
            >
              Shopping Cart
            </h1>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
              ...
              <div className="lg:col-span-2 space-y-4 lg:space-y-6">
                {items.map((item, index) => (
                  <div
                    key={item.product.id}
                    className="flex flex-col sm:flex-row gap-4 sm:gap-6 pb-4 sm:pb-6 border-b border-gray-100 animate-in fade-in slide-in-from-bottom-4 duration-500 fill-mode-both"
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <Link href={`/products/${item.product.id}`}>
                        <h3 className="font-medium text-[#111827] hover:text-[#888] transition-colors">
                          {item.product.name}
                        </h3>
                      </Link>
                      <p className="text-sm text-[#6B7280] mt-1 capitalize">{item.product.category}</p>
                      <p className="text-[#111827] font-medium mt-2">â‚¨{item.product.price.toLocaleString()}</p>
                      ...
                      <div className="flex items-center gap-4 mt-4">
                        ...
                        <button
                          ...
                          className="text-sm text-[#6B7280] hover:text-red-500 focus:text-red-500 transition-colors flex items-center gap-1 group focus:outline-none focus-visible:ring-2 focus-visible:ring-red-400 rounded"
                        >
                          <svg className="w-4 h-4 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 sm:group-focus:opacity-100 transition-opacity" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="lg:col-span-1 order-first lg:order-last">
                <div
                  className="bg-[#F8F7F5] rounded-lg p-6 sticky top-24 animate-in fade-in slide-in-from-bottom-4 duration-500 delay-200 fill-mode-both"
                >
                  ...
                  <div className="space-y-4 mb-6">
                    ...
                    <div className="flex justify-between text-[#6B7280]">
                      <span>Shipping</span>
                      <span>{totalPrice >= 5000 ? 'Free' : 'â‚¨500'}</span>
                    </div>
                    <div className="border-t border-gray-200 pt-4 flex justify-between text-[#111827] font-medium">
                      <span>Total</span>
                      <span>â‚¨{(totalPrice + (totalPrice >= 5000 ? 0 : 500)).toLocaleString()}</span>
                    </div>
                  </div>
